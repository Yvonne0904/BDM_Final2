...
df_pattern = df_pattern.filter(F.col("placekey").isin(placeList)).cache()

df_pattern = df_pattern.select("placekey","poi_cbg","visitor_home_cbgs", date_format("date_range_start", "yyyy-MM").alias("Formatted_start_date"),date_format("date_range_end", "yyyy-MM").alias("Formatted_end_date"))
dateList = ['2019-03','2019-10','2020-03','2020-10']
df_pattern = df_pattern.filter(F.col("Formatted_start_date").isin(dateList) | F.col("Formatted_end_date").isin(dateList))
...
df_pattern = df_pattern.withColumn('date',dateUdf('Formatted_start_date','Formatted_end_date'))
...
df_pattern = df_pattern.withColumn('res_key',keyUdf("visitor_home_cbgs")).withColumn('res_value',valueUdf("visitor_home_cbgs"))

df_pattern = df_pattern.withColumn("new", F.arrays_zip("res_key", "res_value"))\
       .withColumn("new", F.explode("new"))\
       .select('date',"poi_cbg", F.col("new.res_key").alias("home_key"), F.col("new.res_value").alias("people_number"))

df_pattern5 = df_pattern.filter(F.col('home_key').substr(1, 2) == '36')
